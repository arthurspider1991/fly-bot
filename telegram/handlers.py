...
def _processar_admin(chat_id, texto, nome, dados, msg_obj=None):
    ...
    if texto == "/financeiro":
        ...
        msg = (
            ...
            f"Comissoes geradas: R$ {r['comissoes_geradas']:.2f}\n"
            f"  Em carteira (parceiros): R$ {r['saldo_parceiros']:.2f}\n"
            f"  Saques pendentes: R$ {r['saques_pendentes']:.2f}\n"
            f"  Saques pagos: R$ {r['saques_pagos']:.2f}\n\n"
            f"Lucro liquido: R$ {r['lucro_liquido']:.2f}\n\n"
            f"Parceiros com saldo: {r['parceiros_com_saldo']}\n\n"
            f"Por plano:\n{linhas_plano}\n"
